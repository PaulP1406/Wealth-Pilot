...
export function BalanceCard({ balance, accounts }: BalanceCardProps) {
  return (
    <div className="bg-[#2a2a2a] rounded-xl p-6 border border-[#3a3a3a] h-fit">
      {/* Clickable section that links to accounts page */}
      <Link
        href="/dashboard/accounts"
        className="block cursor-pointer hover:bg-[#353535] transition-colors rounded-lg p-2 -m-2"
      >
        ...
        <div className="mb-6">
          ...
          <div className="space-y-3">
            {accounts.map((account, index) => (
              <div
                key={index}
                className="flex items-center justify-between p-3 bg-[#3a3a3a] rounded-lg"
              >
                <div className="flex items-center space-x-3">
                  <div
                    className={`w-4 h-4 rounded-full ${
                      account.active ? "bg-green-500" : "bg-gray-500"
                    }`}
                  ></div>
                  <div>
      ...
      {/* Monthly Goal Section - NOT clickable */}
      <div className="border-t border-[#3a3a3a] pt-6">
        ...
        <div className="text-sm text-gray-400">
          Forecasted capital is $39,968.60
        </div>
      </div>
    </div>
  );
